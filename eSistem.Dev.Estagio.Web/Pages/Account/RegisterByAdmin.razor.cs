...
{
    public partial class RegisterByAdminPageCode : ComponentBase
    {
        #region Dependências
        ...
        private bool OnSubmitRegisterValidator()
        {
            UserValidator = new();
                ...
                case Core.Enums.ETipo.Fisico:
                    {
                        if (InputModel.CpfCnpj.Length != 11)
                            UserValidator.CpfCnpjError.Add($"O {tipoMessage} deve conter no máximo 11 digitos.");
                    }
                    break;
                ...
                case Core.Enums.ETipo.Juridico:
                    {
                        if (InputModel.CpfCnpj.Length != 14)
                            UserValidator.CpfCnpjError.Add($"O {tipoMessage} deve conter no máximo 14 digitos.");
                    }
                    break;
            }
            ...
            string adminName = AuthenticationStateProvider.GetAuthenticadedUser()?.UserName ?? string.Empty;
            ...
            if (InputModel.UserName == adminName)
                UserValidator.UserNameError.Add($"O nome do usuário deverá ser único e não pode ser igual ao seu nome, {adminName}");
